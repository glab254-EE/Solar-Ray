...
public class CustomEnemyAltarBehaviour1 : MonoBehaviour, IDamagable
{
    ...
    [field:SerializeField]
    private string AltarTimerText = "";
    [field:SerializeField]
    private GeneralPurposeEventBehaviour activator;
    ...
    private void Start()
    {
        activator.connections += Activate;        
    }
    void Update()
    {
        if (activating && !spawningActive && !WaveFinished && !prologeShown)
        {
        ...
        HoverOverChecker.LabelText = AltarTimerText+Mathf.Round(EnemiesSpawnDuration-MainTimer)+" секунды.";
        ...
        {
            if (taskManager != null) 
                taskManager.currentTaskMessage = AltarTimerText+Mathf.Round(EnemiesSpawnDuration-MainTimer)+" секунды.";;
        }
